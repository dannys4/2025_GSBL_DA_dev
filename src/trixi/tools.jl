...
function ensemble_to_itp(ensemble, sys::TrixiSystem; ode_transforms::Union{Nothing, <:NamedTuple} = nothing)
    x_quad = Trixi.allocate_coefficients(Trixi.mesh_equations_solver_cache(sys.semi)...)
    ...
    for t_idx in axes(ensemble, 2)
        vec2sol!(x_quad, @view(ensemble[:, t_idx]), sys.equations; g = (x, eqns) -> prim2cons(to_solver_transform(x), eqns))
...
function get_plot_ensemble(ensemble::Matrix{Float64}, sys::TrixiSystem; ode_transforms::Union{Nothing, <:NamedTuple} = nothing)
    Ne = size(ensemble, 2)
    ...
    Nvar = nvariables(sys.equations)
    ensemble_itp = ensemble_to_itp(ensemble, sys; ode_transforms)
    ...
    for ens_idx in axes(ensemble_plot, 3)
        ...
        data = map(x -> sys.dg.basis.Vp * x,
            StructArrays.components(map(x->from_solver_transform(cons2prim(x, sys.equations)), member_itp))
        )
