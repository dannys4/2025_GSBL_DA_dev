...
WikiGeneralizedGamma(a::Integer, d::Integer, p::Integer) =
    WikiGeneralizedGamma(float(a), float(d), float(p))
...
# 2) sampler(dist::Distribution)
Distributions.sampler(rng::AbstractRNG, dist::WikiGeneralizedGamma) =
    Base.rand(rng::AbstractRNG, dist::WikiGeneralizedGamma)
...
function Distributions.logpdf(dist::WikiGeneralizedGamma{T}, x::Real) where {T<:Real}
    ...
    if x <= 0
        ...
        return log(abs(p)) - d * log(a) - loggamma(d / p) + (d - 1) * log(x) - (x / a)^p
