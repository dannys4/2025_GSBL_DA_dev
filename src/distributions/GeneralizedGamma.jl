...
GeneralizedGamma(r::Integer, β::Integer, ϑ::Integer) =
    GeneralizedGamma(float(r), float(β), float(ϑ))
...
# 1) rand(::AbstractRNG, dist::UnivariateDistribution)
Base.rand(rng::AbstractRNG, dist::GeneralizedGamma) =
    rand(rng, wikiGG_from_generalizedGG(dist))

# 2) sampler(dist::Distribution) 
Distributions.sampler(rng::AbstractRNG, dist::GeneralizedGamma) =
    Base.rand(rng::AbstractRNG, dist::GeneralizedGamma)

# 3) logpdf(dist::UnivariateDistribution, x::Real) 
Distributions.logpdf(dist::GeneralizedGamma, x::Real) =
    logpdf(wikiGG_from_generalizedGG(dist), x)

# 4) cdf(dist::UnivariateDistribution, x::Real) 
Distributions.cdf(dist::GeneralizedGamma, x::Real) = cdf(wikiGG_from_generalizedGG(dist), x)

# 5) quantile(dist::UnivariateDistribution, q::Real) 
Distributions.quantile(dist::GeneralizedGamma, q::Real) =
    quantile(wikiGG_from_generalizedGG(dist), q)
...
# 8) insupport(dist::UnivariateDistribution, x::Real) 
Distributions.insupport(dist::GeneralizedGamma, x::Real) =
    insupport(wikiGG_from_generalizedGG(dist), x)
